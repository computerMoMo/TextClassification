...
def load_data(configs):
    ...
    for (tag, file_name) in file_to_tag_list:
        ...
        if os.path.isfile(text_file):
            ...
            raise ValueError(text_file + " not exist")
...
def generate_train_valid_data(configs, data_lists, word_to_id_dicts, char_to_id_dicts, num_classes):
    x_data = []
    y_data = []
    ...
    for (text_list, char_list, tag) in data_lists:
        x_item = []
        for word in text_list:
...
if __name__ == "__main__":
    ...
    # 生成train和valid data
    x_train_data, y_train_data, x_valid_data, y_valid_data = generate_train_valid_data(configs, data_lists, word_to_id_dicts, char_to_id_dicts, len(file_to_tag_dict))
